...
// takes in center, scale, and rotation representing rotation about y in degrees
  ...
  static setObjData(mesh: any) {
    for(let i = 0; i < mesh.vertices.length; i+=3) {
        Roof2.vertices = Roof2.vertices.concat([[mesh.vertices[i], mesh.vertices[i + 1], mesh.vertices[i + 2], 1]]);
        Roof2.normals = Roof2.normals.concat([[mesh.vertexNormals[i], mesh.vertexNormals[i + 1], mesh.vertexNormals[i + 2], 0]]);
    }
  ...
  create() {
    ...
    for(let i = 0; i < Roof2.vertices.length; i++) {
        let pos = vec4.create();
        let nor = vec4.create();
        pos = vec4.fromValues(Roof2.vertices[i][0], Roof2.vertices[i][1], Roof2.vertices[i][2], 1);
        nor = vec4.fromValues(Roof2.normals[i][0], Roof2.normals[i][1], Roof2.normals[i][2], 0);
        pos[0] *= this.scale[0];
        pos[1] *= this.scale[1];
        pos[2] *= this.scale[2];
